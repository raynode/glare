  ...
  return (args: Dictionary<any>) => {
    ...
    const where = reduce(args, (where, value, key) => {
      ...
      const [field, modifier] = splitIndex === -1
        ? [key, 'eq']
        : [key.substr(0, splitIndex), key.substr(splitIndex + 1)]
