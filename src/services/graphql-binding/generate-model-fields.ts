...
export type FieldGroupsUpdater = (
  groups: Dictionary<FieldGroup>,
  name: string,
  group: FieldGroup,
...
export const attributeModifiersToFieldsModifier = (attributeModifiers: AttributeModifier[]): FieldsModifier => (
  ...
  reduce(
    fields,
    (fields: Dictionary<FieldGroup>, field) =>
      attributeModifiers.reduce(
        (fields, attributeModifier) => ({
          ...fields,
...
export const allAttributes: AttributeModifier = ({ attribute, field, name }) =>
  toFieldDictonary({
    name,
    attribute,
    field,
...
export const fieldGenerator = (fields: Dictionary<FieldGroup>, config: BuildConfiguration) => (
  ...
  fieldsModifiers: FieldsModifier[] = [],
) =>
  groupsToFields(
    reduce(
...
export const toListFieldGroup = (field: FieldGroup): FieldGroup => ({
  ...field,
  field: { ...field.field, type: toGraphQLList(field.field.type) },
})

export const requiredAttributeModifier: AttributeModifier = ({ attribute, field, name }) =>
  toFieldDictonary({
    name,
    attribute,
    field:
      attribute.allowNull || attribute.defaultValue !== undefined
        ? field
        : { ...field, type: nonNullGraphQL(field.type) },
  })

export const listAttributeTypeModifier: AttributeModifier = ({ attribute, field, name }, config) =>
  toFieldDictonary({
    name,
    attribute,
...
export const stringAttributeModifier: AttributeModifier = (field, config) =>
  !config.isStringAttribute(field.attribute.type)
    ? toFieldDictonary(field)
    : {
...
export const listAttributeModifier: AttributeModifier = (field, config) =>
  !config.isListAttribute(field.attribute.type)
    ? toFieldDictonary(field)
    : {
        ...toFieldDictonaryWithNameDesc(
          toListFieldGroup(field),
          `${field.name}_in`,
          `${field.name} is found in this list`,
        ),
        ...toFieldDictonaryWithNameDesc(
          toListFieldGroup(field),
          `${field.name}_not_in`,
          `${field.name} is not found in this list`,
        ),
      }

export const arithmeticAttributeModifier: AttributeModifier = (field, config) =>
  !config.isArithmeticAttribute(field.attribute.type)
    ? toFieldDictonary(field)
    : {
        ...
        ...toFieldDictonaryWithNameDesc(
          field,
          `${field.name}_gte`,
          `${field.name} is greater or equal than this value`,
        ),
      }
...
export const createFilterModifier = (fieldFilter: (field: FieldGroup) => boolean): FieldsModifier => fields =>
  filterFields(fields, fieldFilter)
...
export const removeInvisibleAttributes: FieldsModifier = fields =>
  filterFields(fields, ({ attribute }) => (attribute.hasOwnProperty('visible') ? attribute.visible : true))
